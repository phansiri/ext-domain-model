...
class MoneyTests: XCTestCase {
    ...
    func testUSDtoEURtoUSD() {
        ...
        XCTAssert(eur.description == "\(eur.currency)\(eur.amount).0")
        XCTAssert(usd.description == "\(usd.currency)\(usd.amount).0")
    ...
    func testUSDtoGBPtoUSD() {
        ...
        XCTAssert(tenUSD.amount == usd.amount)
        XCTAssert(tenUSD.currency == usd.currency)
        XCTAssert(gbp.description == "\(gbp.currency)\(gbp.amount).0")
        XCTAssert(usd.description == "\(usd.currency)\(usd.amount).0")
    }
    func testUSDtoCANtoUSD() {
        ...
        XCTAssert(can.description == "\(can.currency)\(can.amount).0")
        XCTAssert(usd.description == "\(usd.currency)\(usd.amount).0")
    }
    ...
    func testAddUSDtoUSD() {
        ...
        XCTAssert(total.description == "\(total.currency)\(total.amount).0")
    }
